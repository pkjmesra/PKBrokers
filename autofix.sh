...
isort . --skip pkbrokers/kite/__init__.py

# Run ruff format excluding the specific file  
ruff format . --exclude pkbrokers/kite/__init__.py
...
find . -name "*.py" -not -path "./pkbrokers/kite/__init__.py" -print0 | while IFS= read -r -d '' file; do
    autopep8 --in-place --select="$SELECT_CODES" --aggressive "$file"
done

# Also exclude the file from flake8 statistics if needed
flake8 . --count --exit-zero --max-complexity=10 --max-line-length=127 --statistics --exclude=pkbrokers/kite/__init__.py
